...
!------------------------------------------------------------------------------!
!
! 3.15  March 2, 2016       Various.
!
! 3.14, September 26, 2011  Fixed an input bug when data file only has MT data
! 
! 3.14, Aug 2011            DGM added 1D/3D inversion.
!                       Noticed that these version numbers don't agree with those
!                       in Occam.f90 - which appears to have been updated instead of here.
!
...
!    resolving thin resistive layers: Geophysics, 74, doi:  10.1190/1.3058434 
!
!
...
!    receivers using orthogonal Procrustes rotation analysis: Geophysics, 75, F63–F70.
!
!------------------------------------------------------------------------------!
! Acknowledgments:
!------------------------------------------------------------------------------!
...
!
! DGM 8/2011 - For 1D/3D - the horizontal "connections" between the vertical stacks of 1D inversions.
!   If nConn is non-zero, then the inversion treats EACH Tx as having its own set of 1D layers
!   and uses nSiteConn and nSiteConnWt to put 1st diff smoothing horizontally between the corresponding
!   layers of each Tx's parameters.
!       This means that the MODEL file will have a list of the layers
!   but the STARTUP / ITER file will have a number of params equal to (# free layers) x (# Tx)
!   I have *NOT* implemented this for 1D MT in this code since MT is Rx specific, not Tx specific.
!   Do NOT mix MT and CSEM in a 1D/3D inversion.
!
    integer                              :: nConn       ! # of site-to-site smoothness penalty connections
    integer                              :: nFreeLayers ! # of free layers (some are fixed):: nParams = nFreeLayers * nTxIn
    integer, dimension(:,:), allocatable :: nSiteConn   ! 2 columns (Tx #1, Tx #2)
    real, dimension(:), allocatable      :: nSiteConnWt ! Weight to use for each Tx pairing
...
! 
    ...
    write(*,*) ' '  
    write(*,*) '                  Copyright 2007-2016                           '
    write(*,*) '                      Kerry Key                                 '
    write(*,*) '           Scripps Institution of Oceanography                  '
    write(*,*) '                     kkey@ucsd.edu                              '
    write(*,*) ''    
    write(*,*) '    CSEM1D.f90:    Version 3.15, March 2, 2016.               '
    write(*,*) '    Dipole1D.f90:  Version 7.3, Feb 10, 2010.               '
    write(*,*) ' '
    write(*,*) '   The algorithms are described in:'
    write(*,*) ' '
    write(*,*) '    Key, K., 2009, One-dimensional inversion of multi-component,    '
    write(*,*) '    multi-frequency marine CSEM data: Methodology and synthetic     '
    write(*,*) '    studies for resolving thin resistive layers: Geophysics, 74,   '
    write(*,*) '    doi:  10.1190/1.3058434.'
    write(*,*) ' '
    write(*,*) '    Key, K. and A. Lockwood, 2010, Determining the orientation of   '
    write(*,*) '    marine CSEM receivers using orthogonal Procrustes rotation  '
    write(*,*) '    analysis: Geophysics, 75, F63–F70.'   
    write(*,*) ' '
    write(*,*) ' '        
        ...
        case ('# receivers','nrx','#receivers')
            ...
            do while (iRxRead < nRxIn) 
                ...
                elseif ( (trim(sDataFormat) == 'emdata_1.1').or.(trim(sDataFormat) == 'emdata_1.2')) then
                    ...
                    ! DGM 08/2011 Don't spew everything to the screen - only need to
                    ! check the first so many.
                    if( iRxRead < 21 ) then
                        write(*,'(3(f12.2,2x),3(a12,2x))')  xRxIn(iRxRead),yRxIn(iRxRead),zRxIn(iRxRead), & 
                              & trim(sFields(4)),trim(sFields(5)),trim(sFields(6))  
                    elseif( iRxRead == 21 ) then
                        write(*,*) 'More than 20 receivers. Not writing the rest.'
                    endif
                endif
...
!
    lerror = .false.
    if ((nTxIn < 1).and.(nd<0)) then   ! KWK: nd < 0 is kludge for case of MT data but no CSEM...need to rethink this...
        lerror = .true.
...
!------------------------------------------------------------------------------
    ...
    ! DGM 8/2011 - if 1D/3D inversion, do NOT use reciprocity
    if (nConn > 0 .or. nTxIn < 2) then
        lTxSame = .false.
    else
        lTxSame = .true.
        do iTx = 2,nTxIn
...
! Local variables:
! 
    integer         :: i, j, err, iof, ilayread, nAllocErr
    character(180)  :: sLine, sCode, sValue ! These are for reading the lines of the file
    logical         :: bComment
...
!
    do while (.true.) 
        ...
        case ('end')    ! DGM 8/2011
            exit    ! special code so comments & stuff can be stashed at the end of the file
            
        case ('format')
            call lower(sValue)
            if ( (trim(sValue) /= 'occam1dmod_1.0') .and.   &  
            &    (trim(sValue) /= 'resistivity1dmod_1.0') ) then
                write(*,*) ' Error: model format unsupported: ', trim(sValue)
        ...
        case ('# layers','#layers', 'number of layers','nlayers')   
            ...
            do while (ilayread < nlay1D) 
                ...
                ! Count how many layers are actually free parameters
                if (layer_params(ilayread,2) == -1) then
                    nFreeLayers = nFreeLayers + 1
                endif
            enddo
            !
            ! Check and make sure all the layers were read in:
        ...
        ! DGM 8/2011 - Add support for 1D/3D inversion - a 3D "field" of 1D inversions 
        ! which are regularized horizontally & vertically. The horizontal regularization
        ! is always 1st diff smoothing. Vertical reg is as specified in the startup file.
        case ('# site connections', 'site connections', '# connections')
            read(sValue,*)  nConn
            write(*,*) '# Site Connections: ', nConn
            if( nConn == 0 ) cycle
            
            if( nlay1D == 0 ) then
                write(*,*) '# Site Connections MUST FOLLOW # layers in model file.'
                stop
            endif
            
            ! Allocate the list of site-to-site smoothness penalty connections
            allocate( nSiteConn(nConn,2), stat=nAllocErr )
            if (nAllocErr == 0) then
                allocate( nSiteConnWt(nConn), stat=nAllocErr )
            endif
            if (nAllocErr .ne. 0) then
            ...
            read( iof, *, end=198, err=199 ) &
                (nSiteConn(i,1), nSiteConn(i,2), nSiteConnWt(i), i=1,nConn)
            
            ! For debugging, write the values back out to the screen
            write(*,*) ' '
            write(*,*) ' Site-to-site regularization'
            write(*,*) 'SiteFrom SiteTo Weight'
            write(*,'(I9,X,I6,X,F8.5)') &
                (nSiteConn(i,1), nSiteConn(i,2), nSiteConnWt(i), i=1,nConn)
...
!
    if ((nConn == 0 .and. nParams /= nFreeLayers ) &
    .or.(nConn /= 0 .and. nParams /= nFreeLayers * nTxIn)) then
        ...
        write(*,*) ' Number of free parameters in iteration file: ', nParams
        write(*,*) ' Number of free layers in model file: ',        nFreeLayers
        if (nConn /= 0) write(*,*) ' Number of 1D/3D connections in model file: ', nConn
        write(*,*) '  Stopping!'        
        stop
    endif
...
!
! Apply reciprocity, if possible.
! DGM 8/2011 - this call used to be at the end of readData, but was moved here
!   so that the optional 1D/3D inversion can make sure that reciprocity is NOT
!   used.
!       This call must occur BEFORE setup1Dlayers since allocations are performed
!   there which rely on variables set in this call.
!
...
!
    ...
    return
198 call filerr( ' Model file ended unexpectedly', iof )
...
!
    ...
    ! For 1D/3D, repeat prejudices & weights for each Tx. They should
    ! all be the same. (no way to specify differently at the moment)
    if( nConn /= 0 ) then
        do i = 2,nTxIn
            premod( (((i-1)*nFreeLayers)+1):(i*nFreeLayers) ) = premod(1:nFreeLayers)
            prewts( (((i-1)*nFreeLayers)+1):(i*nFreeLayers) ) = prewts(1:nFreeLayers)
        enddo
    endif
...
!
    ...
    allocate (ipm2ilay(nFreeLayers), stat=nAllocErr)
    if (nAllocErr .ne. 0) then
        write(*,*) 'Out of memory.  Too many parameters (', nFreeLayers, ')'
        stop 
    endif           
! 
! Set up a few things:
    ...
    do i=1,nlay1D
        ...
        !
            if (log10(sig1D(i)) > 12) then
                sig1d(i) = 1d12
              ! write(*,*) ' warning, resistivity too low, moving back up to 1d-12 for layer: ',i
            endif
            if (log10(sig1D(i)) < -12) then
                sig1d(i) = 1d-12
               !write(*,*) ' warning, resistivity too high, moving back down to 1d12 for layer: ',i
            endif           
...
! 
    if (nTxIn > 0 ) then   ! Only call this if there is CSEM data (since we could just have MT data)
    	call computeFwd_CSEM( bDoPartials, ipm2ilay, currentMod )
    endif
...
!============================================================! computeFwd_CSEM !  
!==============================================================================!  
    subroutine computeFwd_CSEM( bDoPartials, ipm2ilay, currentMod )
!
! Routine to compute the 1DCSEM forward response and model Jacobian matrix
!
...
!
    logical, intent(in)         :: bDoPartials         ! logical flag for computing Jacobians  
    integer, intent(in)         :: ipm2ilay(nFreeLayers) 
    real(RealPrec), intent(in)  :: currentMod(nParams)   ! input model parameters
!
! Local variables:
!
    ...
    integer :: iWJ(nFreeLayers)
        ! DGM 8/2011 added for 1D/3D since the range of WJ affected
        ! by each Tx is different in this regime.
...
! 
    do iTx = 1,nTxTfm
        if (bDoPartials .and. nConn /= 0) write (*,'(i3,$)') iTx
!
...
!
! DGM Aug 2011 - if doing 1D/3D inversion, then re-load the model
!   for each Tx. Assume data are gathered for ONE TX per 1D model 
!   space and that these models are then regularized horizontally
!   based on Tx-to-Tx connections specified in the model file.
!
        if (nConn > 0) then
            j = 0
            do i = 1, nlay1D
                ! Just do free params. Fixed are already done.
                if (layer_params(i,2) < 0) then
                    j = j + 1
                    
                    ! NB: No need to update ipm2ilay() - it is already set properly.
                    
                    ! Convert the log10(resistivity) parameters to conductivity
                    sig1D(i) = 1.d0 / (10**currentMod(j + (nFreeLayers * (iTx-1))))
                endif
            enddo
            
            ! Enforce hard limits on sigma
            where ( sig1D > 1d12 ) sig1D = 1d12
            where ( sig1D < 1d-12 ) sig1D = 1d-12
            
            ! Update which parameters in wj are affected by the data from this Tx
            do i = 1,nFreeLayers
                iWJ(i) = i + (iTx-1)*nFreeLayers
            enddo
        endif
            ...
            do i=1,nd
              ...
              iRx = RxInd( dpTfm(i,2))                  
               ...
               ! Electric: Real and Imaginary Data:
                  ...
                  case (indRealEx)  
                    wj(i,iWJ) = dble(dexdsig(iRx,ipm2ilay))    !        real(Ex)
                  case (indImagEx)  
                    wj(i,iWJ) = aimag(dexdsig(iRx,ipm2ilay))   !        imag(Ex)
                  case (indRealEy)  
                    wj(i,iWJ) = dble(deydsig(iRx,ipm2ilay))    !        real(Ey)
                  case (indImagEy)  
                    wj(i,iWJ) = aimag(deydsig(iRx,ipm2ilay))   !        imag(Ey)
                  case (indRealEz)  
                    wj(i,iWJ) = dble( djzdsig(iRx,ipm2ilay))   !        real(Ez)
                  case (indImagEz)  
                    wj(i,iWJ) = aimag( djzdsig(iRx,ipm2ilay))  !        imag(Ez)
                                  
               ! Electric: Amplitude and Phase Data:
                                     
                  case (indAmpEx)   
                    wj(i,iWJ) = absDeriv( ex1d(iRx), dexdsig(iRx,ipm2ilay) )   !     |Ex|
                  case (indAmpEy)   
                    wj(i,iWJ) = absDeriv( ey1d(iRx), deydsig(iRx,ipm2ilay) )   !     |Ey|
                  case (indAmpEz)   
                    wj(i,iWJ) = absDeriv( jz1d(iRx), djzdsig(iRx,ipm2ilay) )   !     |Ez|
                  case (indPhsEx) 
                    wj(i,iWJ) = phaseDeriv( ex1d(iRx), dexdsig(iRx,ipm2ilay) )   !     Ex phase 
                  case (indPhsEy)
                    wj(i,iWJ) = phaseDeriv( ey1d(iRx), deydsig(iRx,ipm2ilay) )   !     Ey phase 
                  case (indPhsEz) 
                    wj(i,iWJ) = phaseDeriv( jz1d(iRx), djzdsig(iRx,ipm2ilay) )   !     Ez phase 

               ! Magnetic: Real and Imaginary Data:

                  case (indRealBx) 
                    wj(i,iWJ) = dble(dbxdsig(iRx,ipm2ilay))   !        real(Bx)
                  case (indImagBx) 
                    wj(i,iWJ) = aimag(dbxdsig(iRx,ipm2ilay))  !        imag(Bx)
                  case (indRealBy) 
                    wj(i,iWJ) = dble(dbydsig(iRx,ipm2ilay))   !        real(By)
                  case (indImagBy) 
                    wj(i,iWJ) = aimag(dbydsig(iRx,ipm2ilay))  !        imag(By)
                  case (indRealBz) 
                    wj(i,iWJ) = dble(dbzdsig(iRx,ipm2ilay))   !        real(Bz)
                  case (indImagBz) 
                    wj(i,iWJ) = aimag(dbzdsig(iRx,ipm2ilay))  !        imag(Bz)
                                  
               ! Magnetic: Amplitude and Phase Data:
                              
                  case (indAmpBx) 
                    wj(i,iWJ) = absDeriv( bx1d(iRx), dbxdsig(iRx,ipm2ilay) )   !     |Bx|
                  case (indAmpBy) 
                    wj(i,iWJ) = absDeriv( by1d(iRx), dbydsig(iRx,ipm2ilay) )   !     |By|
                  case (indAmpBz) 
                    wj(i,iWJ) = absDeriv( bz1d(iRx), dbzdsig(iRx,ipm2ilay) )   !     |Bz|
                  case (indPhsBx) 
                    wj(i,iWJ) = phaseDeriv( bx1d(iRx), dbxdsig(iRx,ipm2ilay) )   !     Bx phase 
                  case (indPhsBy) 
                    wj(i,iWJ) = phaseDeriv( by1d(iRx), dbydsig(iRx,ipm2ilay) )   !     By phase 
                  case (indPhsBz) 
                    wj(i,iWJ) = phaseDeriv( bz1d(iRx), dbzdsig(iRx,ipm2ilay) )   !     Bz phase 
             
               ! Polarization Ellipse Parameters:   
               
                  case (iPEmax)
                    wj(i,iWJ) = getPEDeriv(ex1d(iRx),ey1d(iRx),dexdsig(iRx,ipm2ilay),deydsig(iRx,ipm2ilay),'pmax') ! E-field max
                  case (iPEmin)
                    wj(i,iWJ) = getPEDeriv(ex1d(iRx),ey1d(iRx),dexdsig(iRx,ipm2ilay),deydsig(iRx,ipm2ilay),'pmin') ! E-field min
                  case (iPBmax)
                    wj(i,iWJ) = getPEDeriv(bx1d(iRx),by1d(iRx),dbxdsig(iRx,ipm2ilay),dbydsig(iRx,ipm2ilay),'pmax') ! B-field max
                  case (iPBmin)
                    wj(i,iWJ) = getPEDeriv(bx1d(iRx),by1d(iRx),dbxdsig(iRx,ipm2ilay),dbydsig(iRx,ipm2ilay),'pmin') ! B-field min
...
! Local variables:
! 
    integer        :: i, nPen, nPar, iSite, iLayer, iPFrom, iPTo
...
! 
    ...
    case( eSumParam )
        ...
        enddo
        nPen = nPar     ! set in case 1D/3D needs it
    ...
    ! DGM 8/2011 - if 1D/3D, then the "vertical" penalties from above must
    !   be repeated for each Tx and then the Tx-to-Tx 1st diff penalties
    !   must be applied.
    if (nConn > 0) then
        ! repeat vertical penalties
        do i = 2,nTxIn
            del( (((i-1)*nPen)+1):(((i-1)*nPen)+nPen)   &
               , (((i-1)*nPar)+1):(((i-1)*nPar)+nPar) ) &
               = del(1:nPen,1:nPar)
            
            if (bHaveLink) then
                linkpr( (((i-1)*nPen)+1):(((i-1)*nPen)+nPen), 1:2 ) &
                    = linkpr(1:nPen,1:2) + (nPar * (i-1))
            endif
        enddo
        
        ! add Tx-to-Tx penalties
        nPen = nPen * nTxIn
        do iSite = 1,nConn
            ! Indicies just before the beginning of the params for each site.
            iPFrom  = (nSiteConn(iSite,1) - 1) * nFreeLayers
            iPTo    = (nSiteConn(iSite,2) - 1) * nFreeLayers
            
            do nPar = 1,nFreeLayers
                nPen = nPen + 1    ! write to which penalty row?
                
                ! fill in the penalty matrix - 1st diff smoothing between "columns" of models
                del(nPen,iPFrom+nPar) = -nSiteConnWt(iSite)
                del(nPen,iPTo+nPar)   =  nSiteConnWt(iSite)
                
                ! if asked for list of parameter linkages, give it.
                if( bHaveLink ) then
                    linkpr(nPen,1) = iPFrom + nPar
                    linkpr(nPen,2) = iPTo + nPar
                endif
            enddo
        enddo
...
!-----------------------------------------------------------------------
    ...
    case( eSumParam )
        nTerms = nFreeLayers
    case( eTwoModel )
        nTerms = nFreeLayers - 2
        if( rRufParam .ne. 0.0 ) nTerms = nTerms * 2
    case default
        nTerms = nFreeLayers - 1
    end select
    
    ! DGM 8/2011 If 1D/3D inversion, the param cnt above is per Tx and
    !   then there are the # of Tx-to-Tx connections times # free layers
    if (nConn > 0) then
        nTerms = (nTerms * nTxIn) + (nConn * nFreeLayers)
    endif
...
! 
    ...
    !! readData
    if (allocated(fTxIn)) deallocate( fTxIn )                   ! frequency list
       
    if (allocated(xTxIn)) then
...
!! readModel - 1D/3D inversion Tx-to-Tx connections
    if( nConn > 0 ) deallocate( nSiteConn, nSiteConnWt )
