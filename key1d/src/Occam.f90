...
! 3.14-3.15 March 2, 2016       KK, DGM     Various bug fixes and improvements from past five years.
!
! 3.13      October, 2011       KK, DGM     Fixed minor bugs associated with the prejudice weights and the objective function. 
!
! 3.12      Aug, 2011           DGM         Added support for 1D/3D inversion - each Tx has its own 1D model and the
!                                           model file specifies horizontal 1st diff smoothing between the corresponding
!                                           1D layers of specific Tx's. Regularization of each Tx's model does not have
!                                           to be 1st diff - it can be any roughness type. But regularization across the
!                                           models is 1st diff to tie them together.
!
! 3.11      May, 2011           DGM         Added support for auto-converge feature I have previously used with Occam2DMT.
!                                           Supercedes the older 'Delta Misfit Limit' feature. See code below for details.
!
...
!                                           flat sensitivity scaling in the pass-band between the bounds.   
!                                           "Model Bounds" uses the transformations that bound the parameters.
!                                           "Model limits" uses DGM's older brute force method for truncating parameters outside the
!                                           limits.  While in many cases the brute force "model limits" option works just fine, the 
!                                           "model bounds" transformation option tends to have better convergence when the model is 
!                                           up against the bounds since the transformation is also applied to the Jacobian.   
!
! 3.9       March 2, 2011       DGM, KK     Fix to allow Min Norm inversion to have 2nd stage in Occam procedure.
...
!
! 3.4       May 1, 2009         KK, DGM     Added some bounds on mu in tofmu.  If mu is outside the bounds,
...
!
! 3.0       Oct/Nov 2006        DGM, KK     Big re-write! Moved to F90, allocated memory, cmdline params,
...
module Occam
    ...
    private ! default is to make things private to this module
...
!
    ...
    integer, public        :: nCurrentIter = 0 ! Current Occam iteration number
    character(180), public :: cRootName = ' '  ! Iteration file root name
    ...
    ! MatLab ignores.
    integer, public                                     :: ioUnitOccamLogFile = 20
    ...
    ! Note: associated data/model parameters for the current iteration 
    ...
    real(RealPrec), public                              :: rDeltaMisfit = 0.0
    ...
    !   I added some new types and the ability to have 2 numbers, whereas KK changed
    ...
    !   to determine at runtime and are now easily readable through the parameters below.
    !
    ...
    ! 7     TwoModel
    integer, parameter, public:: e1stDeriv = 1     ! 1st derivative penalty
    integer, parameter, public:: e2ndDeriv = 2     ! 2nd deriv -- Not all Occam uses support this
    integer, parameter, public:: eDepthWt  = 4     ! 1st deriv * log10(depth)
    integer, parameter, public:: eSumParam = 5     ! DGM 3/2010 sum of model params (e.g. log10(rho))
                                        ! In 1D, tends to make thin sheet models (!!)
    integer, parameter, public:: eMinGrad  = 6     ! DGM 3/2010 Minimum gradient support (aka weighted L2)
    ...
    integer, parameter, public:: eTwoModel = 7     ! DGM 10/2010: 1D anistropy attempt 
    ...
    integer, public         :: iRufType = e1stDeriv
    real(RealPrec), public  :: rRufParam = 0.0
    ...
    ! DGM 5/25/2011 - copied from Occam2DMT where I had a much better implementation
    !   of auto-convergence than the simple thing implemented here.
    !
    ! Variables read from iter file controlling new "auto converge" feature.
    ! Automatically converge if delta misfit between iter i and i-N is less 
    ! than THRESHOLD. Set new target RMS to current rms * FACTOR, set converged
    ! flag and continue running.
    !   Optionally auto-converge if have cut step-size max # of times and
    ! can't continue (i.e. konv = 2)
    ! Format of line: iIterCnt, rThreshold, rFactor, bKonvEq2
    ! Example: 'Auto converge: 2, 0.1, 1.1, 1'
    ! DGM 12/2010
    logical, public     :: bAutoConv = .false., bAConkonvEq2 = .false.
    integer, public     :: iAC_IterCnt = 0
    real(4), public     :: rAC_Threshold = 0.0, rAC_Factor = 0.0
...
!
    ...
    integer, private                                     :: nFracTimes = 8  ! # of times to cut stepsize before giving up
    integer, private                                     :: idebug = 0      ! current debug level
    ...
    logical, private        ::  gbModelLimited = .false., gbModelStepped = .false.
    real(RealPrec), private ::  gnModelMin = 0, gnModelMax = 0, gnModelSteps = 0

    ! KWK March 2011
    ! Non-linear transformation for constraining parameters with upper and lower bounds:
    logical, public                    :: lConstrainParams = .false.
    real(RealPrec), parameter, private :: bandPassfactor = 15.  ! factor in exp(bandPassFactor/(a-b) * x)
    character(80), private    :: cBoundsTransform = 'bandpass'
		! 'exponential' or 'bandpass'. The default is set here
...
!---------------------------------------------------------------------
    ...
    public :: deallocateOccam, transformToBound
...
!
! KWK 04/15/2011 - Rewritten so that it appends instead of overwriting if 
!                  starting inversion from a previous iteration
!
! DGM Aug 2009 - changed from ioUnitOccamLogFile param to variable for
!                MatLab interface
!
    integer       :: ierr
    logical       :: lFileExists
    ...
    if ( (nCurrentIter > 0).and.(lFileExists) ) then
      ...
      ! Drop a line letting the user know this is a restart:
        ...
        write(ioUnitOccamLogFile,*) ' *** Occam restarted here from iteration: ',nCurrentIter
        write(ioUnitOccamLogFile,*) ' ---------------------------------------- '
    ...
    if (ierr .ne. 0) then
        write(*,*) ' Error opening log file:', trim(cFileName)
        stop 
    end if
...
123     continue
    ...
    ! DGM 5/25/2011 - Below is a MUCH IMPROVED auto-convergence facility that
    ! I created for the Occam2DMT and have only just copied here. Use it in
    ! place of the older "delta misfit limit" stuff.
    case ('auto converge')      ! DGM 12/2010
        bAutoConv   = .true.
        ! Format of line: iIterCnt, rThreshold, rFactor, bKonvEq2
        ! bKonvEq2 should be 0 (false) or non-zero (true)
        ! The commas are required.
        i  = 1
        i2 = index( sValue(i:), ',' )
        if( i2 == 0 ) call error_on_autoconverge
        read(sValue(i:i2-1),*) iAC_IterCnt
        
        do i=i2+1,len_trim(sValue)          ! Skip comma & space padding
            if (sValue(i:i) .ne. ' ') exit
        enddo
        i2 = index( sValue(i:), ',' ) + i - 1
        if( i2 == 0 ) call error_on_autoconverge
        read(sValue(i:i2-1),*) rAC_Threshold
        ...
        i2 = index( sValue(i:), ',' ) + i - 1
        if( i2 == 0 ) call error_on_autoconverge
        read(sValue(i:i2-1),*) rAC_Factor
        
        do i=i2+1,len_trim(sValue)          ! Skip comma & space padding
            if (sValue(i:i) .ne. ' ') exit
        enddo
        bAConkonvEq2 = ( sValue(i:i) /= '0' )
        
        write(*,*) '  Auto Converge when RMS(i-', iAC_IterCnt &
                 , ') - RMS(i) < ', rAC_Threshold
        if( bAConkonvEq2 ) then
            write(*,*) '  OR if have convergence problems.'
        else
            write(*,*) '  BUT NOT if have convergence problems.'
        end if
        write(*,*) '  Set new target to RMS(i) * ', rAC_Factor
    
        
    case ('model limits')   ! DGM Nov 2006 see module  Occam_internal_mod
        ! Limit value string must have two values: min, max
        ! Comma is required.
        i = index( sValue, ',' )
        if (i == 0) call error_on_modellimits
        read(sValue(:i-1),*) gnModelMin
        
        ! Skip space padding
        do i=i+1,len_trim(sValue)
            if (sValue(i:i) .ne. ' ') exit
        enddo
         read(sValue(i:),*) gnModelMax
        
        ! Min & Max must be reasonable
        if (gnModelMax < gnModelMin .or. gnModelMax == gnModelMin) then
            call error_on_modellimits
        endif
        
        gbModelLimited = .true.
        write(*,'(a27,g12.5,a4,g12.5)') 'Limiting model values from ', gnModelMin, ' to ', gnModelMax
    ...
    case ('model bounds')   ! KWK March 2011: Now using true bounds transformations...
        ! Limit value string must have two values: min, max
        ! Comma is required.
        i = index( sValue, ',' )
    ...
    if (lConstrainParams) then
       ...
       ! Make sure pm is inside the bounds (nudged just inside bounds if not since exactly at bounds goes to +-infinity):
        pmtol = (upperBound - lowerBound) / 232
        where (pm <= lowerBound + pmtol) pm = lowerBound + pmtol
        where (pm >= upperBound - pmtol) pm = upperBound - pmtol
        ! now transform it:
...
!-----------------------------------------------------------------------
!-----------------------------------------------------------------------
    subroutine error_on_modellimits
    ...
    end subroutine  error_on_modellimits 

!-----------------------------------------------------------------------
!-----------------------------------------------------------------------
    subroutine error_on_autoconverge
    
    implicit none
    
    write(*,*) 'Invalid "Auto Converge" entry in startup file.'
    write(*,*) 'The format for this line is:'
    write(*,*) '  Auto Converge: N, Threshold, Factor, StepSizeFlag'
    write(*,*) 'Commas required.'
    write(*,*) 'See code comments for meaning of values.'
    stop
            
    end subroutine  error_on_autoconverge
         
!-----------------------------------------------------------------------
!-----------------------------------------------------------------------
    subroutine error_on_bounds
    ...
    end subroutine   error_on_bounds
    ...
    if (lConstrainParams) then ! KK's new model bounds supercedes DM's old model limits
    ...
    if (bAutoConv) then
        if( bAConkonvEq2 ) then
            write(ioUnitOccamIterFile,'(A,I2,A,F9.6,A,F9.6,A)') &
                      'Auto Converge:      ', iAC_IterCnt    &
                    , ', ', rAC_Threshold               &
                    , ', ', rAC_Factor                  &
                    , ', 1'
        else
        ...
        write(ioUnitOccamIterFile,*) '!Auto Converge: N,Threshold,Factor,KonvEq2Flag'
    endif
    ...
    character(50), intent(inout) :: cStartup
    character(180), intent(inout):: cRootName
    ...
    ! DGM 2/15/2012 root name wasn't being handled properly.
    if( len_trim(cStartup) == 0 ) then
        cStartup = 'startup'
        cRootName = 'ITER'
    elseif( len_trim(cRootName) == 0 ) then
        n = index(cStartup,'.')
        if( n > 0 ) then
            cRootName = cStartup(1:n-1)
        else
            cRootName = cStartup
        endif
    endif
...
! DEBUG!
!          open (unit=ioUnitOccamIterFile, file='wjtwj.dump')
!          write(ioUnitOccamIterFile,'(1x,E15.8)') wjtwj
!          close(ioUnitOccamIterFile)
! DEBUG!
...
! DEBUG!
!          open (unit=ioUnitOccamIterFile, file='wjtwd.dump')
!          write(ioUnitOccamIterFile,'(1x,E15.8)') wjtwd
!          close(ioUnitOccamIterFile)
! DEBUG!
...
! DEBUG!
!        open (unit=ioUnitOccamIterFile, file='deltdel.dump')
!        write(ioUnitOccamIterFile,'(1x,F12.8)') ptp
!        close(ioUnitOccamIterFile)
!        
!        open (unit=ioUnitOccamIterFile, file='linkpr.dump')
!        write(ioUnitOccamIterFile,'(1x,I4)') linkpr
!        close(ioUnitOccamIterFile)
! DEBUG!
    ...
    ! DGM 2011 - the prejudice term was not being added to the roughness.
    ! Now add on the norm for the prejudice model (which is 0 if not used):
    fndruf = fndruf + sum( (prewts*(premod - pwk2))**2 )
    ...
    pwk4  = wjtwd + amu*(prewts**2)*premod  ! KWK Oct 2011: fixed missing **2 on prewts.  prewts is usually 0 or 1, so not a big deal
    amat  = 0.0
    ...
    if (abs(idebug) >= 1) then
        write(*,'(a,f8.4,A,g12.5,A,f9.3,A,f9.3)') '  Misfit: ', tofmu, '  Mu: ', amu, &
             & '  Min/max rho: ',10**minval(pwk2),', ',10**maxval(pwk2)
	endif
